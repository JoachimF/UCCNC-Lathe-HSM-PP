/**
  Copyright (C) 2024-2024 by Autodesk, Inc.
  All rights reserved.

  UCCNC Lathe post processor configuration.
...
description = "UCCNC Turning";
vendor = "UCCNC";
vendorUrl = "http://www.cncdrive.com/";
longDescription = "Turning post for UCCNC. 1st try";
...
properties = {
  diametermode: {
    title      : "Diameter mode",
    description: "X coordinates in diameter values",
    group      : "formats",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useG28Path: {
    title      : "G28 use intermediate coord.",
    description: "Enable G28 intermediate coord.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  homePathXG28: {
    title      : "G28 path position X",
    description: "G28 X-axis path position. (Work Coord.) ",
    group      : "homePositions",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  homePathZG28: {
    title      : "G28 path position Z",
    description: "G28 Z-axis path position.(Work Coord.)",
    group      : "homePositions",
    type       : "number",
    value      : 0,
    scope      : "post"
...
function onOpen() {
  ...
  if (getProperty("diametermode")) {
	...
	writeBlock(gFormat.format(8)); // Radius mode
  }
...
function onCyclePoint(x, y, z) {
  ...
  case "thread-turning":
    if (isLastCyclePoint()) {
      ...
      writeBlock(
        gMotionModal.format(76), //G76 P... Z... I... J... K... E... L... Q... H..
        "P" + spatialFormat.format(cycle.pitch), //The pitch of the thread in distance units per spindle revolution.
        zOutput.format(z),
        "I" + xFormat.format(threadClearance), //The thread peak position in units along the X-axis. This is the starting height of the thread and the material was mostly turned down to this size
        "J" + xFormat.format(firstDepthOfCut), //Cut depth per pass. The first cut will be J beyond I along the X-axis
        "K" + xFormat.format(threadHeight), //Full thread depth. The last cut will be this deep along the X-axis.
        //"R" + spatialFormat.format(threadCuttingMode), //
        "Q" + spatialFormat.format(cuttingAngle), //Slide in angle. Specifies an angle between the cuts. As the tool goes from one cut depth to the other the cut start is offset along the Z-axis with this angle measured from the X-axis. Programming
        "H" + spatialFormat.format(repeatPass), //Number of final cuts. To make any extra cuts on the final depth program this parameter other than 0. The value of the parameter will define the number of extra cuts on the final depth. If the parameter is not set the default value of 0 will be used causing a single final depth cut.
        "E" + spatialFormat.format(chamferWidth), //Taper length. Specifies the length of the taper(s). For a 45° taper program the E parameter equal to the K parameter
...
function writeRetract() {
  ...
  var _yHome;
  var _zHome;
  if (method == "G28") { // homePathXG28 useG28Path
    //_xHome = toPreciseUnit(0, MM);
    //_yHome = toPreciseUnit(0, MM);
    //_zHome = toPreciseUnit(0, MM);
	_xHome = getProperty("homePathXG28");
    _yHome = toPreciseUnit(0, MM);
    _zHome = getProperty("homePathZG28");
  } else {
  ...
  if (arguments.length > 0) {
    ...
    case "singleLineXZ":
	  ...
	  for (var i = 0; i < retractAxes.length; ++i) {
		...
		case X:
		  if(getProperty("useG28Path") || method == "G53")
    ...
    if(!getProperty("useG28Path") && method == "G28")
		words.push("");
	
  for (var i = 0; i < words.length; ++i) {
    switch (method) {
    case "G28":
      if(getProperty("useG28Path"))
